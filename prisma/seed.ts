...
async function main() {
  ...
  const admin = await prisma.user.upsert({
    where: { email: 'admin@aquatrack.com' },
    update: {},
    create: {
      name: 'Admin User',
  ...
  const driver1 = await prisma.user.upsert({
    where: { email: 'driver@aquatrack.com' },
    update: {},
    create: {
      name: 'Ahmed Driver',
  ...
  const driver2 = await prisma.user.upsert({
    where: { email: 'driver2@aquatrack.com' },
    update: {},
    create: {
      name: 'Saeed Driver',
      email: 'driver2@aquatrack.com',
      password: driverPass,
      phone: '+971503456789',
      role: 'driver',
    },
  ...
  const customer1 = await prisma.user.upsert({
    where: { email: 'customer@aquatrack.com' },
    update: {},
    create: {
  ...
  const product1 = await prisma.product.upsert({
    ...
    create: {
      id: 'prod-1',
      name: 'Standard Water 18L',
      description: 'Pure drinking water, 18 liters',
      price: 5,
      liter: 18,
      stock: 200,
    },
  ...
  const product2 = await prisma.product.upsert({
    ...
    create: {
      id: 'prod-2',
      name: 'Premium Mineral 18L',
      description: 'Premium mineral water, 18 liters',
      price: 8,
      liter: 18,
      stock: 150,
    },
  ...
  const product3 = await prisma.product.upsert({
    ...
    create: {
      id: 'prod-3',
      name: 'Standard Water 5L',
      description: 'Pure drinking water, 5 liters',
      price: 2.5,
      liter: 5,
      stock: 300,
    },
  ...
  const product4 = await prisma.product.upsert({
    ...
    create: {
      id: 'prod-4',
      name: 'Premium Mineral 5L',
      description: 'Premium mineral water, 5 liters',
      price: 4,
      liter: 5,
      stock: 250,
    },
  ...
  for (let i = 1; i <= 70; i++) {
    ...
    bottles.push({
      qrCode: `AQUA-${String(i).padStart(4, '0')}`,
      status,
      refillCount,
      maxRefills: 30,
    });
  ...
  const order1 = await prisma.order.create({
    data: {
      ...
      items: {
        create: [
          { productId: product1.id, quantity: 3, unitPrice: 5 },
        ],
      },
    },
  });
  ...
  const order2 = await prisma.order.create({
    data: {
      ...
      items: {
        create: [
          { productId: product2.id, quantity: 2, unitPrice: 8 },
        ],
      },
    },
  });
  ...
  const order3 = await prisma.order.create({
    data: {
      ...
      items: {
        create: [
          { productId: product1.id, quantity: 2, unitPrice: 5 },
        ],
      },
    },
  });
  ...
  await prisma.promoOffer.upsert({
    ...
    create: {
      code: 'SAVE5',
      description: 'AED 5 off orders over AED 20',
      type: 'fixed',
      value: 5,
      minOrder: 20,
      maxUses: 50,
